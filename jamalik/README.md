...
| `RUN_SEED` | — | `true` يفرض التعبئة. غير لازم عادة: البناء يتعرّف وحده على أول نشر |
| `RESET_ADMIN_PASSWORD` | — | `true` يعيد كلمة مرور المدير إلى `SEED_ADMIN_PASSWORD` — **احذفيه بعدها** |
...
2. سجّلي الدخول إلى `/admin` وغيّري كلمة المرور من `/admin/account`.
   ولإضافة من يحرّر معك: **الحسابات ← حساب جديد**.
   وإن ضاعت كلمة مرور المدير الوحيد، انظري `RESET_ADMIN_PASSWORD` أعلاه.
